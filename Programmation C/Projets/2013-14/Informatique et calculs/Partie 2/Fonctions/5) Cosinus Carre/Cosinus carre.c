...
int main()
{
    ...
    {
        printf("Quelle est la borne minimale de l'intervalle de valeurs ? (entre -6 et 6) "); /// La valeur est automatiquement en radian
    ...
    {
        printf("Quelle est la borne maximale de l'intervalle de valeurs ? (entre -6 et 6, > borne minimale) "); /// La valeur est automatiquement en radian
    ...
    for (x = mini ; x <= maxi ; x = x + pas) /// Boucle de calcul
    {
        ...
        tabx [n] = x;
        x1 = x;
        x = x * x; /// Association à x son carré
    ...
    f=fopen("Cosinus carre.csv", "w"); /// Ouverture du fichier où seront écrits les couples
